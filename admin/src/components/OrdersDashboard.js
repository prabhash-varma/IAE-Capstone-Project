...
useEffect(()=>{
    ...
    if(search===""){
    Axios.get("https://ays-mern-backend.onrender.com/getordersforadmin",{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}}).then((res)=>{
...
const updateList = () => {
    Axios.get(`https://ays-mern-backend.onrender.com/filterordersforadmin/?filter=${filter}&search=${search}`,{headers:{"authorization":`bearer ${localStorage.getItem("token")}`}}).then((response) => {
